...
export default function TeamSection() {
  return (
    <section
      id="team"
      className="py-24 px-8 md:px-16 lg:px-24 ml-0 md:ml-20 border-b border-zenith-surface"
    >
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {team.map((member, index) => (
            ...
            >
              <div className="relative aspect-square overflow-hidden border border-zenith-surface bg-zenith-surface mb-4 rounded-lg">
                <Image
                  src={member.image}
                  alt={member.name}
                  fill
                ...
                {/* Status Indicator */}
                <div className="absolute top-2 right-2 flex items-center gap-2 bg-zenith-base/80 backdrop-blur-sm px-2 py-1 border border-zenith-surface rounded-md">
                  <div
                  ...
                  />
                  <span className="text-[10px] font-mono uppercase text-zenith-text/70">
                    {member.status}
                  </span>
                </div>
              </div>

              <h3 className="text-xl font-heading font-bold uppercase mb-1 text-zenith-text group-hover:text-zenith-cyan transition-colors">
                {member.name}
              </h3>
              ...
              <div className="flex gap-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 transform translate-y-2 group-hover:translate-y-0">
                <a
                  ...
                  className="text-zenith-text/50 hover:text-zenith-text transition-colors"
                >
